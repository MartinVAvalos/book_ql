...
var unref = maybeRef => typeof maybeRef === 'function' ? maybeRef() : maybeRef != null && vue.isRef(maybeRef) ? maybeRef.value : maybeRef;
...
function callUseQuery(_args, client = useClient(), stops = []) {
  ...
  var isPaused = vue.ref(!!unref(args.pause));
  var input = vue.shallowRef({
    request: core.createRequest(unref(args.query), unref(args.variables)),
...
function callUseMutation(query, client = useClient()) {
  ...
  return {
    ...
    executeMutation(variables, context) {
      ...
      })(client.value.executeMutation(core.createRequest(unref(query), unref(variables)), context || {})))));
    }
  };
}
  ...
  stops.push(vue.watchEffect(onInvalidate => {
    if (source.value) {
      ...
      onInvalidate(wonka.subscribe(result => {
        fetching.value = true;
        data.value = result.data != null ? typeof scanHandler.value === 'function' ? scanHandler.value(data.value, result.data) : result.data : result.data;
        error.value = result.error;
        extensions.value = result.extensions;
        stale.value = !!result.stale;
        operation.value = result.operation;
